...
class MultivariateGaussian(RandomVariable):
        ...
        if name is "mean":
            if isinstance(value, (int, float)):
                ...
                assert value is None, (
                    "mean must be either"
                    "int, float, np.ndarray, MultivariateGaussian, or None")
    ...
    def _map(self, X):
        assert isinstance(self.mean, MultivariateGaussian)
    ...
    def _bayes(self, X):
        N = len(X)
        mean_is_gaussian = isinstance(self.mean, MultivariateGaussian)
        ...
        if mean_is_gaussian and precison_is_ndarray:
            ...
            self.mean = MultivariateGaussian(
                mean=S @ (
